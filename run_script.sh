...
echo "running script on $1 hosts"
...
echo "$public_ips" | while read i; do
  ...
  scp -i "$ssh_key_file" -o "StrictHostKeyChecking=no" "$2" "$ssh_username"@"$i":/home/"$ssh_username"/myScript.sh
  ssh -n -i "$ssh_key_file" "$ssh_username"@"$i" "./myScript.sh"
done